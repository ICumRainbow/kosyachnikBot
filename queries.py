...
# the query I used when worked with pymysql
# SELECT_LAST_TIME_QUERY = 'SELECT CONVERT_TZ(last_time,"-04:00", "+05:00") AS "last_time" FROM contest_groups WHERE id=%s'

# the query I'm using to work with psycopg2
SELECT_LAST_TIME_QUERY = 'SELECT last_time AS "last_time" FROM contest_groups WHERE id=%s'
...
# the query I'm using to work with psycopg2
SELECT_CHAT_ID_QUERY = 'SELECT EXISTS(SELECT chat_id FROM scores WHERE chat_id=%s);'
...
# the query I used when worked with pymysql
# SELECT_STATS_QUERY = '''SELECT COALESCE(users.username, users.name) AS "name"
#                    FROM scores
#                    JOIN users ON scores.chat_id = %s AND scores.user_id=users.id;'''

# the query I'm using to work with psycopg2
SELECT_STATS_QUERY = '''SELECT EXISTS(SELECT COALESCE(users.username, users.name) AS "name"
                     FROM scores
                     JOIN users ON scores.chat_id = %s AND scores.user_id=users.id);'''

SELECT_ID_QUERY = "SELECT EXISTS(SELECT id from users where id=%s)"
